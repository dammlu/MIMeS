...
OTU_table_bulk <- read.csv("rarefied_bulk_bootstrap.csv")  %>% column_to_rownames("X") %>% t()%>% as.data.frame()
...
for (k in 1:19) {
  file_path <- file.path("/scratch/ldamm/Paper_eDNA_bulk/initial clustering analyses/initial analyses for clustering similarities/R_initial_analyses/Mangan18_Paper_final/Final_bootstrap"
                         , file_names[k])
